...
export default function RestaurantCard({
  ...
  useEffect(
    () =>
      favoritesService.subscribe((ids) =>
        setIsFavorite(ids.includes(restaurant.id)),
      ),
    [restaurant.id],
  );
  const rating = restaurant.rating_percent
    ? (restaurant.rating_percent / 20).toFixed(1)
    : "4.7";
  const deliveryLabel =
    restaurant.delivery_fee === 0
      ? "Livraison offerte"
      : `Livraison ${Number(restaurant.delivery_fee).toFixed(0)} DH`;
    ...
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.9}>
      <View style={styles.imageContainer}>
        <Image
          ...
          resizeMode="cover"
        />
        {(restaurant.promo_badge || restaurant.delivery_fee_promo) && (
          <View
            style={[
              styles.promoBadge,
              !restaurant.promo_badge && styles.freeBadge,
            ]}
          >
        ...
        <View style={styles.deliveryTimePill}>
          <Clock3 color={Colors.textPrimary} size={13} strokeWidth={2.5} />
          <Text style={styles.deliveryTimePillText}>
        ...
        >
          <Heart
            size={18}
            color={isFavorite ? Colors.secondary : Colors.textPrimary}
      ...
      <View style={styles.infoSection}>
        ...
        <View style={styles.ratingRow}>
          <Star color={Colors.primary} fill={Colors.primary} size={13} />
...
const styles = StyleSheet.create({
  ...
  promoBadgeText: { color: Colors.white, fontSize: 11, fontWeight: "800" },
  deliveryTimePill: {
    position: "absolute",
    bottom: 10,
    left: 10,
  ...
  infoSection: { paddingHorizontal: 14, paddingVertical: 13 },
  restaurantName: {
    fontSize: 16,
    fontWeight: "800",
    color: Colors.textPrimary,
  ...
  starText: { fontSize: 13, fontWeight: "800", color: Colors.primaryDeep },
  ratingCount: { fontSize: 12, color: Colors.textMuted },
  metaDot: { fontSize: 12, color: Colors.textMuted, marginHorizontal: 3 },
  cuisineText: {
    fontSize: 12,
